...
async def init_audit_pool(
    ...
    *,
    ...
    try:
        ...
        logger.warning(
            "audit.pool.init_failed", extra={"error_type": type(exc).__name__}
        )
...
class AuditPublisherNotConfigured(RuntimeError):
    """Raised when legacy audit is called without an activity-backed publisher."""
...
async def publish_audit_event(
    *,
    ...
    """Build an AuditEvent and publish it through the configured activity path.

    Legacy audit DB fallback has been intentionally removed. Calls require an
    explicit audit publisher or a configured default ``ActivityRecorder``.

    If a custom ``IAuditPublisher`` has been set (via ``set_audit_publisher``),
    it takes precedence. Otherwise, a configured activity recorder is used via
    ...
    try:
        ...
        # Explicit audit publishers win; otherwise prefer TAP-38 activity when configured.
        publisher = _audit_publisher or _get_activity_audit_publisher()
        if publisher is None:
            raise AuditPublisherNotConfigured(
                "Legacy audit DB fallback is disabled; configure ActivityRecorder"
            )
    ...
    except AuditPublisherNotConfigured:
        raise
...
def _get_activity_audit_publisher() -> IAuditPublisher | None:
    from taproot_common.activity import get_activity_recorder
    from taproot_common.audit.activity_adapter import ActivityAuditPublisher

    recorder = get_activity_recorder()
    if recorder is None:
        return None
    return ActivityAuditPublisher(recorder)
